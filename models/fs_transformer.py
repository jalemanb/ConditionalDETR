...
class FSTransformer(nn.Module):
    ...
    def forward(self, src, templates, mask, template_mask, query_embed, pos_embed):
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src,
                ...
                template_padding_mask: Optional[Tensor] = None,
                pos: Optional[Tensor] = None):
        output = src

        for layer in self.layers:
            output = layer(src=output, templates=templates, src_mask=mask,
                           src_key_padding_mask=src_key_padding_mask,
                           template_padding_mask=template_padding_mask, pos=pos)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward_post(self,
        ...
        src2 = self.cross_attn(src, templates, value=templates, key_padding_mask=template_padding_mask)[0]
    ...
    def forward_pre(self, src,
        ...
        src2 = self.cross_attn(src2, templates, value=templates, key_padding_mask=template_padding_mask)[0]
    ...
    def forward(self, src,
        ...
        if self.normalize_before:
            return self.forward_pre(src, templates, src_mask, src_key_padding_mask, template_padding_mask, pos)
        return self.forward_post(src, templates, src_mask, src_key_padding_mask, template_padding_mask, pos)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
        ...
        tgt = self.norm2(tgt)
        template_features = tgt[:num_templates]
        object_features = tgt[num_templates:]

        tgt_obj = self.linear2(self.dropout1(self.activation(self.linear1(object_features))))
        tgt_templ = self.linear4(self.dropout2(self.activation(self.linear3(template_features))))
        tgt = torch.concatenate([tgt_templ, tgt_obj], dim=0)
